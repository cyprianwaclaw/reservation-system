<template>
    ...
    <div class="flex w-full justify-end pr-[30px] pb-[21px] mt-[60px]" :class="isLoading == true ? 'test-window' : ''">
        <CalendarInput v-model="displayDate" />
    </div>
    <div class="main-window" :class="isLoading == true ? 'test-window' : ''">
...
const weekDays = computed(() => {
    ...
    const allDays = Array.from({ length: 60 }).map((_, i) => {
        const d = mondayThisWeek.add(i - 30, 'day');
        return {
